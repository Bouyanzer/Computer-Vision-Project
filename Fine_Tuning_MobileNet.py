...
from Fine_Tuning_MobileNet_function import Binary_MobileNetV2_FineTunning
...
#**************************************************************************
print("\n\t[INFO] MobileNet_FineTunning...")
MyModel=Binary_MobileNetV2_FineTunning(input_shape,optimizer,ClassificationType,lossV,NbClass)
MyModel.summary()
#**************************************************************************
H=list(MyModel.metrics_names)# this list is empty it caused issues to the following code
#training
print("\n\t[INFO] Training...")
early_stopping = EarlyStopping(monitor='val_loss', patience=1)#or monitor='val_acc'
mc = ModelCheckpoint('Model/MobileNet_FT_balanced_VAL.h5', monitor='val_accuracy', mode='max', verbose=1, save_best_only=True)
...
SaveHistory(history,'MobileNet_FT')
PlotFigures(history,MyModel,'MobileNet_FT')
validation_steps = (test_set.classes.shape[0]//batch_size+1)#+1 Obligatoire pour la matrice de confusion
ConfutionMatrix_ClassificationReport(MyModel,'MobileNet_FT',test_set,validation_steps)
